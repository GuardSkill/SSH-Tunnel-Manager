...
[windows版本下载](https://github.com/GuardSkill/SSH-Tunnel-Manager/releases/download/v2.0/SSHTunnelManager.exe)
[windows v1版本下载(只支持密钥登陆)](https://github.com/GuardSkill/SSH-Tunnel-Manager/releases/download/v1.0/SSHTunnelManager.exe)
...
- 📋 **主机复制** - 快速复制现有主机配置，轻松创建相似配置
- 🔌 **端口映射** - 轻松创建和管理多个本地到远程端口的映射
- 📊 **全局可视化** - 三区域布局，同时查看所有主机和隧道状态
- 🔄 **批量操作** - 一键启动/停止所有隧道
- 💾 **配置持久化** - 自动保存所有配置，下次启动自动加载
- 🌍 **多语言支持** - 支持中文和英文界面切换
- 🔒 **双重认证** - 支持密码和SSH密钥两种认证方式（支持默认密钥）
- 🚀 **高性能加载** - 启动即可用，快速显示所有隧道配置
- 📌 **系统托盘** - 最小化到系统托盘，后台运行
- 🎯 **纯Python实现** - 使用Paramiko库，无需外部SSH客户端
...
A: 当前版本不支持自动重连。如果隧道断开，需要手动重新启动。自动重连功能计划在未来版本添加。
